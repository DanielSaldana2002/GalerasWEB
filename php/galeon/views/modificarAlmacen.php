...
<head>
    ...
    <title>Modificar almacen | Galeon</title>
</head>
<body>
    <script>
    ...
    <div id="boxRegistro">
        <h1>Modificacion de almacen</h1>
        <form action="/php/galeon/controllers/verificacionModificacionAlmacen.php" method="post" id="formularioRegistro">
            <?php
            session_start();
            $id = $_POST['idA'];
            $stmt = "209.126.107.8";
            ...
            while($row=sqlsrv_fetch_array($res)){
                ...
                echo <<<TEXTO
                    ...
                    <input onkeydown="bloquearBotones()" type="number" name="cantidadAlmacen" id="" placeholder="Cantidad" value="$cantidadA" disabled>
                    <input onkeydown="bloquearBotones()" type="number" name="contenidoAlmacen" id="" placeholder="Contenido" value="$contenidoA" required>
                TEXTO;
            }
            ?>   
            <select name="tipoPiezas" id="">
                <?php
                    if($tipoA == 1){
                        ...
                        $opc=array("Database"=>"galerasw_galeras", "UID"=>"galerasw_galeras2023","PWD"=>"20021120"); 
                        $con=sqlsrv_connect($stmt,$opc) or die(print_r(sqlsrv_errors(), true));
                        $sql="SELECT nombre_tipo_pieza FROM dbo.almacen_tipo ORDER BY id_tipo_piezas ASC";
                        $res=sqlsrv_query($con,$sql);
                        while($row=sqlsrv_fetch_array($res)){
                        ...
                        $opc=array("Database"=>"galerasw_galeras", "UID"=>"galerasw_galeras2023","PWD"=>"20021120"); 
                        $con=sqlsrv_connect($stmt,$opc) or die(print_r(sqlsrv_errors(), true));
                        $sql="SELECT nombre_tipo_pieza FROM dbo.almacen_tipo ORDER BY id_tipo_piezas DESC";
                        $res=sqlsrv_query($con,$sql);
                        while($row=sqlsrv_fetch_array($res)){
                            $tipoPieza = $row['nombre_tipo_pieza'];
                            echo <<<TEXTO
